...
const MybidsCard = ({ bid, getData }) => {
  ...
  const handleComplete = async (id, prevStatus, status) => {
    if (prevStatus === 'In Progress') {
      try {
        const { data } = await axiosSecure.patch(`/bid-seller/${id}`, {
          status,
        });
  ...
  return (
    <tr>
      ...
      <td className="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
        <div
          className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2  text-white  
          ${category === 'Web Development' && 'bg-blue-500'}
          ${category === 'Digital Marketing' && 'bg-pink-200'}
          ${category === 'Graphics Design' && 'bg-yellow-200'}
          `}
        >
